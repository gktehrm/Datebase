...
BACKEND_GUARD_RUNS_URL = "http://localhost:5000/guard-runs"
#DB랑 연결하기
def build_rule_base_reason(review_actions):
    if not review_actions:
        return "Only low-risk actions detected."
    return "Review-required semantic actions detected."


def build_approval_status(final_decision):
...
def build_guard_run_payload(
    ...
    reasoning_text = "\n\n".join(intent.get("reasoning", []))

    payload_actions = []
    for index, action in enumerate(actions, start=1):
        payload_actions.append({
            "action_order": index,
            "agent_name": action.get("agent_name", "claude"),
            "event_id": action.get("event_id"),
            "created_at": action.get("created_at"),
            "syscall": action.get("syscall"),
            "path": action.get("path"),
            "argv": action.get("argv"),
            "raw_summary": action.get("raw_summary"),
            "summary": action.get("summary"),
            "meaning": action.get("meaning"),
            "normalized_action": action.get("normalized_action"),
            "target_class": action.get("target_class"),
            "rule_result": action.get("rule_result"),
        })

    return {
        "user_id": None,
        "agent_name": intent.get("agent_name", "claude"),
        "user_prompt": intent["user_prompt"],
        "ai_agent_reasoning": reasoning_text or None,
        "rule_base_result": rule_base_result,
        "rule_base_reason": build_rule_base_reason(review_actions),
        "guard_llm_result": None if guard_result is None else guard_result["label"],
        "guard_llm_reason": None if guard_result is None else guard_result["reason"],
        "final_decision": final_decision,
        "approval_status": build_approval_status(final_decision),
        "actions": payload_actions,
    }


def save_guard_run(payload):
    response = requests.post(
        BACKEND_GUARD_RUNS_URL,
        json=payload,
        timeout=10,
    )
...
def persist_guard_run(
    ...
    payload = build_guard_run_payload(
        intent=intent,
        actions=actions,
    ...
    try:
        save_result = save_guard_run(payload)
        print("\n--------------------------------------------------")
        print("[DB SAVE]")
        print("Saved guard run id :", save_result["run"]["id"])
        print("Saved actions      :", len(save_result.get("actions", [])))
...
def run():
    ...
    for intent in intents:
        ...
        intent["agent_name"] = get_agent_name_from_syscalls(syscalls)
        ...
        rule_base_result = "REVIEW_REQUIRED"
